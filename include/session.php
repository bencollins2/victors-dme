...
class Session
{
   ...
   function register($subfirst, $sublast, $subpass, $subemail, $lnk){
      ...
      /* Username error checking */
      $field = "first";  //Use field name for username

      if(!$subfirst || strlen($subfirst = trim($subfirst)) == 0){
         $form->setError($field, "* First name not entered");
      }
      else{
         /* Spruce up username, check length */
         $subfirst = stripslashes($subfirst);
         if(!eregi("^([0-9a-z])+$", $subfirst)){
            $form->setError($field, "* First name not alphanumeric");
         }
         /* Check if username is reserved */
         else if(strcasecmp($subfirst, GUEST_NAME) == 0){
            $form->setError($field, "* First name reserved word");
         }
         /* Check if First name is already in use */
         else if($database->usernameTaken($subfirst)){
            $form->setError($field, "* First name already in use");
         }
         /* Check if First name is banned */
         else if($database->usernameBanned($subfirst)){
            $form->setError($field, "* First name banned");
         }
      }

      $field = "last";  //Use field name for username
      if(!$sublast || strlen($sublast = trim($sublast)) == 0){
         $form->setError($field, "* Last name not entered");
      }
      else{
         /* Spruce up username, check length */
         $sublast = stripslashes($sublast);
         if(!eregi("^([0-9a-z])+$", $sublast)){
            $form->setError($field, "* Last name not alphanumeric");
         }
         /* Check if username is reserved */
         else if(strcasecmp($sublast, GUEST_NAME) == 0){
            $form->setError($field, "* Last name reserved word");
         }
         /* Check if Last name is already in use */
         else if($database->usernameTaken($sublast)){
            $form->setError($field, "* Last name already in use");
         }
         /* Check if Last name is banned */
         else if($database->usernameBanned($sublast)){
            $form->setError($field, "* Last name banned");
         }
      }
