<?php
	...
	if ($type == "getmessages") {
		header('Content-type: application/json');
		$query = "SELECT m.id AS 'mid', m.published AS published, SUBSTR(m.from, 2) AS 'from', SUBSTR(m.to, 2) AS 'to', m.message, m.timestamp, u.id, CONCAT(a.first, ' ', a.last) AS fromName, CONCAT(u.first, ' ', u.last) AS toName, m.img1, m.img2, m.img3, m.img4, m.img5, m.img6, m.img7, m.img8, m.img9, m.img10, a.avatar_sm FROM `messages` AS m LEFT JOIN `users` AS u ON SUBSTR(m.to, 2) = u.id LEFT JOIN `adminusers` AS a ON SUBSTR(m.from, 2) = a.id WHERE SUBSTR(m.to, 2) LIKE '$userid' AND published = 1 UNION SELECT m.id, m.published AS published, SUBSTR(m.from, 2) AS 'from', SUBSTR(m.to, 2), m.message, m.timestamp, u.id, CONCAT(u.first, ' ', u.last), CONCAT(a.first, ' ', a.last), m.img1, m.img2, m.img3, m.img4, m.img5, m.img6, m.img7, m.img8, m.img9, m.img10, u.avatar_sm FROM `messages` AS m LEFT JOIN `users` AS u ON SUBSTR(m.from, 2) = u.id LEFT JOIN `adminusers` AS a ON SUBSTR(m.to, 2) = a.id WHERE SUBSTR(m.from, 2) LIKE '$userid' AND published = 1 ORDER BY `timestamp` ASC";
		$result = mysql_query($query);
